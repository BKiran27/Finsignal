...
export const Header: React.FC<{ onSignOut: () => void; userName?: string }> = ({ onSignOut, userName }) => {
  ...
  return (
    <header className="h-[52px] flex items-center surface-0 border-b border-b1 flex-shrink-0 relative z-50" style={{ backdropFilter: 'blur(20px)' }}>
      ...
      <div className="flex items-center gap-2.5 px-3 md:px-5 h-full border-r border-b1 whitespace-nowrap">
        ...
        <span className="font-serif text-base text-brand hidden sm:inline">FinSignal Capital</span>
        <span className="font-serif text-base text-brand sm:hidden">FS</span>
        <span className="text-[9px] font-bold text-brand tracking-widest uppercase ml-1 px-1.5 py-0.5 rounded bg-brand-dim hidden sm:inline">PRO</span>
      </div>
      ...
      <div className="flex-1 md:hidden flex items-center justify-center">
        <div className="flex items-center gap-1.5 text-[10px] text-t2 font-medium">
          <div className="w-1.5 h-1.5 rounded-full bg-fs-green animate-pulse-dot" />
          NSE · Live
        </div>
      </div>

      <div className="flex items-center border-l border-b1">
        <div className="hidden sm:flex items-center gap-1.5 px-4 h-[52px] text-[11px] text-t2 font-medium border-r border-b1">
          <div className="w-1.5 h-1.5 rounded-full bg-fs-green animate-pulse-dot shadow-[0_0_8px_hsl(var(--fs-green)/0.2)]" />
          NSE · Live
        </div>
        {userName && (
          <div className="hidden sm:flex items-center gap-2 px-4 h-[52px] text-[11px] text-t2 border-r border-b1">
        ...
        <button onClick={onSignOut} className="h-[52px] px-3 md:px-4 flex items-center text-t2 hover:text-t0 transition-colors text-xs font-medium">
