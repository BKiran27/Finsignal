...
const AIDeepResearch: React.FC<{ stock: Stock }> = ({ stock }) => {
  ...
  const RESEARCH_MODES = [
    ...
    { id: 'valuation', ic: '💰', label: 'Valuation Matrix', query: `Multi-method valuation analysis for ${stock.s} at ₹${stock.p}. P/E: ${stock.pe}x, P/B: ${stock.pb}x, ROE: ${stock.roe}%, Market Cap: ${stock.mc}. Run DCF, peer multiples, PEG, margin of safety calculations.` },
    { id: 'forensic', ic: '🔬', label: 'Financial Forensics', query: `Forensic financial analysis for ${stock.s}. Current metrics — ROE: ${stock.roe}%, D/E: ${stock.de}, P/E: ${stock.pe}x. Analyse Altman Z-score, Piotroski F-score, cash conversion, working capital quality, and red flags.` },
    { id: 'moat', ic: '🏰', label: 'Moat & Durability', query: `Competitive moat analysis for ${stock.s} in ${stock.sec} sector. Rate across 5 dimensions (brand, switching costs, network effects, cost advantage, IP). Score /50. Assess durability over next decade.` },
    { id: 'risk', ic: '⚠️', label: 'Risk Register', query: `Comprehensive risk analysis for ${stock.s}. Beta: ${stock.beta}, Pledge: ${stock.pledge}%, D/E: ${stock.de}. Build probability × impact matrix for all risk categories. Estimate max drawdown.` },
    { id: 'catalyst', ic: '⚡', label: 'Catalyst Calendar', query: `Catalyst analysis for ${stock.s} (${stock.sec}). Identify all upcoming catalysts: earnings dates, capex milestones, regulatory events, sector tailwinds, AGM. Timeline next 12 months with probability-weighted impact.` },
  ];
    ...
    <div className="surface-2 border border-b1 rounded-2xl overflow-hidden">
      <div className="px-4 py-3 border-b border-b0 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="text-[11px] font-bold uppercase tracking-wider text-t2">AI Deep Research Engine</div>
        </div>
        <span className="text-[9px] px-2 py-0.5 rounded bg-brand-dim text-brand font-bold">INSTITUTIONAL GRADE</span>
      </div>
      <div className="p-4">
        ...
        {analysis ? (
          <div className="surface-3 border border-b0 rounded-xl p-4 max-h-[400px] overflow-y-auto">
            <div className="text-[12.5px] leading-[1.75] text-t1 whitespace-pre-wrap font-[system-ui]">
              {analysis.split(/(\*\*[^*]+\*\*)/).map((part, i) =>
                part.startsWith('**') && part.endsWith('**')
                  ? <span key={i} className="font-bold text-brand text-[11px] uppercase tracking-wider block mt-3 mb-1.5">{part.slice(2, -2)}</span>
  ...
  return (
    <div className="surface-2 border border-b1 rounded-2xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-b0">
        ...
        <div className="text-[11px] font-bold uppercase tracking-wider text-t2">Quantitative Scorecard</div>
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div key={ma.label} className="surface-3 rounded-lg p-2.5 text-center border border-b0">
        ...
        <div className="surface-3 rounded-lg p-3 border border-b0">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            {[
              { l: 'Resistance 2', v: (s.p * 1.06).toFixed(0), c: 'text-down' },
              { l: 'Resistance 1', v: (s.p * 1.03).toFixed(0), c: 'text-down' },
              { l: 'Support 1', v: (s.p * 0.97).toFixed(0), c: 'text-up' },
              { l: 'Support 2', v: (s.p * 0.94).toFixed(0), c: 'text-up' },
            ].map(lv => (
  ...
  return (
    <div className="surface-2 border border-b1 rounded-2xl overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="flex gap-3 text-[9px] text-t2 font-medium">
      ...
      <div className="p-4">
        <div className="grid grid-cols-3 gap-1.5 mb-3">
          {[
            ...
            { l: 'EV/EBITDA', v: `${evEbitda.toFixed(1)}x`, sub: 'Estimated' },
            { l: 'PEG Ratio', v: pegRatio.toFixed(2), sub: pegRatio < 1 ? '✦ Undervalued' : pegRatio > 2 ? '⚠ Expensive' : 'Fair' },
            { l: 'FCF Yield', v: `${fcfYield}%`, sub: parseFloat(fcfYield) > 4 ? 'Strong' : 'Moderate' },
            { l: 'Div Yield', v: `${s.dy}%`, sub: s.dy > 2 ? 'Good income' : 'Growth focus' },
          ].map(item => (
            <div key={item.l} className="surface-3 rounded-lg p-2.5 text-center border border-b0">
              <div className="font-mono text-[12px] font-semibold mb-0.5">{item.v}</div>
              <div className="text-[9px] text-t3 font-medium">{item.l}</div>
              <div className="text-[8px] text-t3 mt-0.5">{item.sub}</div>
            </div>
          ))}
        </div>
        <div className="surface-3 rounded-lg p-3 border border-b0">
          <div className="text-[9px] text-t3 font-semibold mb-1.5 uppercase tracking-wider">Fair Value Range (Est.)</div>
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col gap-0">
      ...
      <div className="p-5 surface-1 border-b border-b1 flex-shrink-0">
        ...
        {/* Signal Banner */}
        <div className="surface-2 border border-b1 rounded-2xl px-5 py-3.5 flex items-center justify-between">
          ...
          <div className="flex gap-5">
            ...
            <div><div className="font-mono text-sm font-medium text-neutral-signal">{((tgt - s.p) / s.p * 100).toFixed(1)}%</div><div className="text-[10px] text-t3 mt-1 font-medium uppercase tracking-wider">Upside</div></div>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-3">
        {/* Fundamental Grid */}
        <div className="grid grid-cols-8 gap-px bg-b1 border border-b1 rounded-xl overflow-hidden">
          {[
            ...
            { l: 'EPS (Est)', v: `₹${(s.p / s.pe).toFixed(1)}` }, { l: 'Book Value', v: `₹${(s.p / s.pb).toFixed(0)}` },
            { l: 'Mkt Cap', v: s.mc }, { l: 'ROCE (Est)', v: `${(s.roe * 0.85).toFixed(1)}%` },
          ].map((stat, i) => (
            <div key={i} className="surface-1 px-3 py-2.5">
              <div className="font-mono text-[12px] font-medium mb-1">{stat.v}</div>
              <div className="text-[9px] text-t3 font-medium">{stat.l}</div>
            </div>
          ))}
        </div>
        ...
        {/* 2-column layout for analysis panels */}
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="px-5 py-3 surface-0 border-b border-b1 flex-shrink-0 relative">
        <div className="relative max-w-[700px] mx-auto">
          ...
          <input
            ...
            placeholder="Deep research on 200+ NSE/BSE stocks — e.g. RELIANCE, TCS, Banking..."
          />
          {query && (
          ...
          {showDropdown && hits.length > 0 && (
            <div className="absolute top-[calc(100%+6px)] left-0 right-0 surface-1 border border-b2 rounded-2xl z-50 max-h-[400px] overflow-y-auto shadow-2xl">
              {hits.map(s => (
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${s.sc === 'buy' ? 'bg-fs-green-dim text-fs-green' : s.sc === 'sell' ? 'bg-fs-red-dim text-fs-red' : 'bg-fs-gold-dim text-fs-gold'}`}>{s.sig}</span>
                        <span className="text-[9px] px-1.5 py-0.5 rounded surface-3 text-t3 font-medium">{s.mc}</span>
                      </div>
      ...
      {selectedStock ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3.5 text-t3">
          ...
          <div className="text-base font-bold text-t2">Institutional-Grade Stock Research</div>
          <div className="text-[13px] text-t3 text-center max-w-[400px] leading-relaxed">
            Search any stock for deep fundamental analysis, AI-powered research, peer comparison, technical structure, and valuation matrix
          </div>
          <div className="flex gap-2 mt-2">
            {['RELIANCE', 'TCS', 'HDFCBANK', 'INFY', 'TATAMOTORS'].map(sym => (
              <button key={sym} onClick={() => { const s = DB.find(x => x.s === sym); if (s) setSelectedStock(s); }}
                className="px-3 py-1.5 rounded-lg border border-b1 text-[11px] font-semibold text-t2 hover:border-brand hover:text-brand hover:bg-brand-dim transition-all">
                {sym}
              </button>
            ))}
          </div>
